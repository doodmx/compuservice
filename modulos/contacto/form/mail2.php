...
global $chartset;
 if($chartset=='iso-8859-1'){
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        $nombre = str_replace(array("\r","\n"),array(" "," ") , strip_tags($_POST["input-25"]));
        $email = filter_var($_POST["input-26"], FILTER_SANITIZE_EMAIL);
        $tel = $_POST["input-27"];
        $msj = $_POST["Message"];
        ...
        if (empty($nombre) OR !filter_var($email, FILTER_VALIDATE_EMAIL) OR empty($tel) OR empty($msj)) {
    ...
    <div>
      ...
      if($BCC1!=''){$headers .= "Bcc: {$BCC1}\r\n";}
        ...
        if($_SERVER['HTTP_HOST']!='localhost'){
          ...
          $success=mysqli_query($mysqli,"INSERT INTO ".$DBprefix."contacto (ip,nombre,email,para,de,tel,titulo,asunto,msj,cat_list,seccion,tabla,adjuntos,visto,status,ID_login,ID_user,visible) VALUES ('{$ip}','{$nombre}','{$email}','{$CoE1}','','{$tel}','{$title}','{$subject}','{$message}','{$cat_list}','{$sec}','','','0','1','1','1','1');") or print mysqli_error($mysqli);
        }
        if($success) {
          if($_SERVER['HTTP_HOST']!='localhost'){          
            $save=mysqli_query($mysqli,"INSERT INTO ".$DBprefix."contacto (ip,nombre,email,para,de,tel,titulo,asunto,msj,cat_list,seccion,tabla,adjuntos,visto,status,ID_login,ID_user,visible) VALUES ('{$ip}','{$nombre}','{$email}','{$CoE1}','','{$tel}','{$title}','{$subject}','{$message}','{$cat_list}','{$sec}','','','0','1','1','1','1');") or print mysqli_error($mysqli);
